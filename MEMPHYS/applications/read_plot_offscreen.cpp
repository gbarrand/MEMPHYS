...
inline bool plot(std::ostream& a_out,
  ...
  inlib::sg::manager_zb mgr;
  inlib::sg::render_zb action(mgr,a_out,ww,wh);
  ...
  if(do_zb_ps) {
    ...
    if(!wps.open_file(zb_ps_file)) {
      ...
      wps.PS_IMAGE(ww,wh,inlib::wps::rgb_4,inlib::sg::render_zb::get_rgb,&action);
      wps.PS_END_PAGE();
      wps.close_file();  
    }
  }
