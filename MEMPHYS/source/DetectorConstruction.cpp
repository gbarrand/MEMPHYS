...
void MEMPHYS::DetectorConstruction::SetRockGeometry() {
  ...
  RockMotherPosition_z = -RockMotherVolLength_z/2.
    ...
    //}
  RockMotherPosition_y -= WCRadius; //GB : else "Rock" is inside the WC tank!
  RockMotherPosition = 
  ...
  RockCentralHallPosition_y -= WCRadius; //GB : else "Rock" is inside the WC tank!
  //}
...
void MEMPHYS::DetectorConstruction::ConstructRock(G4LogicalVolume *logicExpHall, G4bool RockIsVisible) {
  ...
  rockwallVisAtt->SetForceWireframe(true);
  rockwallVisAtt->SetForceAuxEdgeVisible(false); //bool ops are hugly with all edges visible in wire frame.
...
void MEMPHYS::DetectorConstruction::FillGeometryTuple() {
  ...
  fAnalysis.m_Geometry_wcRadius = WCCylInfo[0];
  fAnalysis.m_Geometry_wcLength = WCCylInfo[1];
  ...
  fAnalysis.m_Geometry_pmtRadius = WCPMTSize;
  fAnalysis.m_Geometry_nPMTs = totalNumPMTs;
  ...
  for ( int tubeID = 1; tubeID <= totalNumPMTs; tubeID++){
    ...
    fAnalysis.m_Geometry_pmtInfos_pmtId = tubeID;
    fAnalysis.m_Geometry_pmtInfos_pmtLocation = cylLocation;
