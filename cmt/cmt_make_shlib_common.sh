...
# kind is shlib or dll. OpenScientist specific.
kind=`${CMTROOT}/mgr/cmt show macro_value ${lib}_kind -tag=${tag}`
...
if test ${extract} = "extract"; then
  ...
  eval ${ld} ${ldflags} -all ${libname}.a -o ${libname}.${suffix} ${extra} -rpath `/bin/pwd`
  result=$?
