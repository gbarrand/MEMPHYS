//
//  This script is a first version to read, with ROOT, a MEMPHYS.root 
// file having the "TTree within TTree" of the MEMPHYS_batch
// progrem (see Analysis.cxx).
//  But the below way to read the data has clearly a HUGE
// memory consumption problem that someone can see by using
// the "top" program on a UNIX.
//
//  Usage :
//    OS> <setup ROOT>
//  ( OS> root batch_dummy.C )
//    OS> root analyis_leaks.C
//
//  G.Barrand
//
...
void analysis_leaks() {
  ...
  for (Int_t i=0; i<nEvent; ++i) {
    ...
    Event_vtxPos->Reset();
    Event_track->Reset();
    Track_direction->Reset();
    Track_momentum->Reset();
    Track_startPos->Reset();
    Track_stopPos->Reset();
    Event_hit->Reset();
    Hit_pe->Reset();
    Event_digit->Reset();
    //tEvent->ResetBranchAddresses();
    */
